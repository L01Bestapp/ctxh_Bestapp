...
export default function UpdateActivityScreen() {
    ...
    useEffect(() => {
        ...
        // Populate form if params exist
        if (params) {
            setActivityName(params.title as string || '');
            setDescription(params.description as string || '');
            setLocation(params.location as string || '');

            // Handle image: params.image from require() is usually a number/string identifier
            // For now, if we can't display the local require easily from param, we might just show placeholder/current.
            // But if it's a URI string (real app), it works direct.
            // Simplified: if mock data passed a number, we can try to skip or handle.
            // Since our mock data uses require(...), passing it via router params might serialize it to a number.
            if (params.image) {
                // If it's a number (ID), we can't easily put it into 'uri' state which expects string.
                // We'll trust the user to re-upload or just leave blank for now to mock "CHANGE YOUR IMAGE".
            }

            // Extract Date/Time if possible - Mock data format: "Oct 20, 2025 - 08:00 AM"
            // We just pre-fill strings for now to match specific mock format logic or just defaults.
            // For simplicity in this demo, let's pre-fill with simple defaults or the exact string if it matches our DD/MM/YYYY format.
            // Our mock data is distinct from form format. I will just set example data if empty.
            if (params.time) {
                // Try to parse "Oct 20, 2025" -> "20/10/2025" is complex without library.
                // We will set dummy data for the purpose of the UI demo as requested.
                setStartDate('20/10/2025');
                setEndDate('25/10/2025');
            }
            if (params.deadline) {
                setDeadline('18/10/2025');
            }
            if (params.slots) {
                const parts = (params.slots as string).split('/');
                if (parts.length === 2) setVolunteers(parts[1]);
            }
        }
    ...
    const [volunteers, setVolunteers] = useState('30'); // Default demo
    const [daysAwarded, setDaysAwarded] = useState('2'); // Default demo
    const [deadline, setDeadline] = useState('');
    const [requirements, setRequirements] = useState('Basic programming knowledge'); // Default
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            >
                ...
                >
                    {/* Image Upload - Modified for Update Screen */}
                    <TouchableOpacity style={styles.uploadContainer} onPress={pickImage}>
                        {/* Show existing image if no new image picked, else show new image. 
                             For demo, we strictly show the design 'CHANGE YOUR IMAGE' overlay logic.
                             We assume a background image exists (mock). 
                         */}
                        <Image
                            source={image ? { uri: image } : require('../assets/images/ob1.png')}
                            style={[styles.uploadedImage, { opacity: 0.6 }]} // Dim the image
                        />
                        <View style={styles.overlayContainer}>
                            <Ionicons name="cloud-upload-outline" size={30} color="#000" />
                            <Text style={styles.uploadTextOverlay}>CHANGE YOUR IMAGE</Text>
                        </View>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity style={styles.createButton} onPress={handleUpdate}>
                        <Text style={styles.createButtonText}>SAVE CHANGE</Text>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    uploadContainer: {
        height: 180,
        backgroundColor: '#FFF', // Changed base
        borderRadius: 12,
